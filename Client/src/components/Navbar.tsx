        ...
        <>
            ...
            >
                ...
                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    ...
                    {
                        isLoggedIn ? <Link to='/my-generations' className="hover:text-pink-300 transition" >My Generations</Link> : <Link to='' className="hover:text-pink-300 transition" >About</Link>
                    }
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                {isLoggedIn ? 
                    <button onClick={()=>{setIsOpen(false); logOut()}}>Logout</button> :  <Link onClick={() => setIsOpen(false)} to='/login'>Login</Link> }
