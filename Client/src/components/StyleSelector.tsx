...
const StyleSelector = ({value,onChange,isOpen,setIsOpen} : {value :ThumbnailStyle ; onChange : (style :ThumbnailStyle)=> void; isOpen:boolean; setIsOpen:(open:boolean)=>void }) => {
    ...
    const styleDescription:Record<ThumbnailStyle,string>={
        ...
        "Minimalist" : "High Contrast,bold typography , striking visuals",
    ...
    return (
        <div className='relative space-y-3 dark'> 
            <label className='block text-sm font-medium to-zinc-200'> Thumbnail Style</label>
            ...
            <button type='button' onClick={()=>setIsOpen(!isOpen)} className='flex w-full items-center justify-between rounded-md border px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12'>
                <div className='space-y-1'>
                    ...
                    <p className='text-xs to-zinc-400'>{styleDescription[value]}</p>
                </div>
                <ChevronDownIcon className={['h-5 w-5 to-zinc-400 transition-transform', isOpen && 'rotate-180'].join(' ')} />
            </button>

            {isOpen && (
                <div className='absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg'>{thumbnailStyles.map((style)=>(
            ...
            <button key={style} type='button' onClick={()=>{onChange(style); setIsOpen(false);}} className='flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30'>
                        ...
                        <div>
                            ...
                            <p className='text-xs to-zinc-400'>{styleDescription[style]}</p>
                        </div>
